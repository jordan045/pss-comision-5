...
import { InscripcionCarreraCreateSchema } from "@/lib/schemas/inscripciones"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!usuario || usuario.rol !== "ALUMNO") {
      return NextResponse.json(
        { message: "Solo los alumnos pueden inscribirse a carreras" },
        { status: 403 }
      )
    }

    const body = await request.json()
    const validatedData = InscripcionCarreraCreateSchema.parse(body)
